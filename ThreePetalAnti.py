...
def generate_ThreePetalAnti(fig, ax, head_color, handle_color, start_side='left', rotation_direction='clockwise'):
    # Parameters
    num_frames = 360
    ...
    # Calculate angles
    theta_1 = np.linspace(start_theta, start_theta -2 * (rotation_direction_multiplier * np.pi), num_frames)
    theta_2 = -2 * theta_1 + (np.pi / 2)
