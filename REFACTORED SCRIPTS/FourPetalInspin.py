...
def generate_FourPetalInspin(fig, ax, head_color, handle_color):
    # Parameters
    num_frames = 360
    ...
    # Calculate angles
    theta_1 = np.linspace(0, 2 * np.pi, num_frames)  # Change to clockwise direction
    theta_2 = 5 * theta_1  # Multiply by 3 for 4 spins per circle (4 - 1)
    ...
    poi_head_trail, = ax.plot([], [], linewidth=1.5, alpha=1, color=head_color)
    poi_handle, = ax.plot([], [], 'yo', markersize=10, alpha=1, color=handle_color)
    poi_handle_trail, = ax.plot([], [], linewidth=1.5, alpha=1, color=handle_color)
