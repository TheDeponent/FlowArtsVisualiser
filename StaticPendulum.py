...
def generate_StaticPendulum(fig, ax, head_color, handle_color, start_side='left', rotation_direction='clockwise'):
    # Parameters
    num_frames = 360
    ...
    theta_1 = np.pi / 2 + amplitude * np.cos(omega * time * swing_direction_multiplier)

    # Calculate coordinates
    poi_head_x = radius * np.cos(theta_1)
    poi_head_y = -radius * np.sin(theta_1)  # Negative sign added to ensure poi head stays at or below handle level
